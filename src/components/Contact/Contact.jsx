...
const Contact = () => {
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    const { data, error } = await supabase
      .from("feedbacks")
      .insert([{ name: feedbackName, message: feedbackMessage }])
      .select();
    ...
    setFeedbacks([
      { id: data[0].id, name: feedbackName, message: feedbackMessage },
      ...feedbacks,
    ]);
          ...
          <div className="mt-8 max-h-64 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gradient scrollbar-track-transparent rounded-2xl bg-black/30 backdrop-blur-md shadow-inner p-2 space-y-4">
            {feedbacks.length === 0 ? (
              ...
              feedbacks.map((fb) => (
                <div
                  key={fb.id}
                  className="flex items-start space-x-4 bg-gray-900/70 p-4 rounded-xl border border-gray-700 hover:border-purple-500 hover:shadow-lg transition transform hover:scale-[1.02]"
                >
